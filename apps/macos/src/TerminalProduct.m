...
+ (BOOL)runWindowSelfTestsWithTheme:(TerminalTheme *)theme
    ...
    [controller refresh];
    BOOL hasSystemNotificationSetting = [controller.items
        indexOfObjectPassingTest:
            ^BOOL(NSDictionary *item, NSUInteger index, BOOL *stop) {
        (void)index;
        (void)stop;
        return [item[@"id"] isEqualToString:@"notifications"];
    }] != NSNotFound;
    BOOL settingsWork =
        controller.items.count == 11 &&
        !hasSystemNotificationSetting &&
        !controller.sectionControl.hidden &&
        !controller.searchField.hidden &&
        !controller.secondaryButton.hidden;
...
- (NSArray<NSDictionary *> *)settingsItems {
    return @[
        ...
        @{@"id":@"ai", @"name":@"AI & Models",
          @"detail":@"Claude subscription or Anthropic API, with separate models"},
...
- (NSArray<NSDictionary *> *)onboardingItems {
    return @[
        ...
        @{@"id":@"api", @"name":@"4  Choose an AI provider",
          @"detail":@"Use a Claude subscription, an Anthropic API key, or keep both available."},
        @{@"id":@"model", @"name":@"5  Default model",
          @"detail":@"Choose a subscription model alias or a live Anthropic API model."},
        @{@"id":@"account", @"name":@"6  Claude Code account",
          @"detail":@"Connect one or more subscription accounts and select one per tab."},
